...
then
	...
	~/cellranger/spaceranger-1.0.0/spaceranger count --id=$5 --fastqs=fastq --transcriptome=~/cellranger/$2 --image=$IMAGE --slide=$SLIDE --area=$AREA
else
	#sometimes people drop the ball and don't save the slide serial number
	~/cellranger/spaceranger-1.0.0/spaceranger count --id=$5 --fastqs=fastq --transcriptome=~/cellranger/$2 --image=$IMAGE --unknown-slide
fi
